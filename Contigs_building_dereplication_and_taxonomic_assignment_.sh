...
cd-hit-est -i denovo/cdhit/allcontigs_filtered.fasta -o denovo/cdhit/allcontigs_filtered_dereplicated.fasta -c 0.95 -G 0 -aS 0.95 -g 1 -r 1 -M 0 -d 0
awk '/^>/{$0=">OTU_"++i}1' denovo/cdhit/allcontigs_filtered_dereplicated.fasta > denovo/cdhit/allcontigs_filtered_dereplicated_OTU.fasta
...
./diamond blastx --query /denovo/cdhit/allcontigs_filtered_dereplicated_OTU.fasta -p 114 -e 0.001 -k 1 --max-hsps 1 --db $db --outfmt 102 --out denovo/cdhit/contigs_LCA_OTU.txt
...
cat denovo/cdhit/allcontigs_filtered_dereplicated_OTU.fasta |awk '/^>/ {if(N>0) printf("\n"); printf("%s ",$0);++N;next;} { printf("%s",$0);} END {printf("\n");}' > denovo/cdhit/allcontigs_lin.fasta
cat diamond/viral_contigs.tsv | awk -F'\t' '{print $2}'|uniq > viral_contigs_list.txt
grep -f viral_contigs_list.txt allcontigs_lin.fasta | sed 's/ /\n/'g > all_viral_contigs.fasta
